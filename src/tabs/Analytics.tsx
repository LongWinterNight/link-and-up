  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      {topInsight && (
        ...
        >
          {t('an.insight.a')}
          <strong>{lbl(topInsight.label)}</strong>
          {t('an.insight.b')}
          <span className="num">{topInsight.avg}</span>
          {t('an.insight.c')}
          {topInsight.n}
          {t('an.insight.d')}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: 16 }}>
        ...
        <Panel title={t('an.hook.title')}>
          <Bars
            caption={t('an.hook.caption')}
            color="var(--positive)"
            items={byHook.map((x) => ({ label: `${lbl(x.label)} (${x.n})`, value: x.avg }))}
          />
        </Panel>

        <Panel title={t('an.struct.title')}>
          <Bars
            caption={t('an.struct.caption')}
            items={byStruct.map((x) => ({ label: `${lbl(x.label)} (${x.n})`, value: x.avg }))}
          />
        </Panel>

        <Panel title={t('an.cta.title')}>
          <Bars
            caption={t('an.cta.caption')}
            color="var(--warning)"
            items={byCta.map((x) => ({ label: `${lbl(x.label)} (${x.n})`, value: x.avg }))}
          />
        </Panel>
        ...
        <Panel title={t('an.scatter.title')}>
          {scatter.length ? (
            <>
              ...
              <div onClick={() => setScatterZoom(true)} style={{ cursor: 'zoom-in' }} title={t('an.scatter.zoomTitle')}>
        ...
        <Panel title={t('an.emotion.title')}>
          <Donut
            caption={t('an.emotion.caption')}
            segments={emotionDist.map((s, i) => ({
              label: lbl(s.label),
              value: s.value,
