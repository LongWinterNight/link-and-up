    ...
    <Modal onClose={() => setOpen(false)} label={t('im.aria')} width={680} zIndex={55}>
      ...
      >
        {(
          ...
          <button
            ...
            style={{
              ...seg,
              ...(mode === m ? { background: 'var(--surface-3)', color: 'var(--text-1)', fontWeight: 600 } : {}),
            }}
          >
            {label}
          </button>
      ...
      {mode === 'json' && (
        <>
          ...
          <input
            ...
            style={{ fontSize: 12.5, marginBottom: 10, color: 'var(--text-2)' }}
            aria-label={t('im.json.file.aria')}
          />
          <textarea
            value={text}
            onChange={(e) => {
      ...
      {mode === 'linkedin' && (
        <>
          <div style={{ fontSize: 12.5, color: 'var(--text-2)', lineHeight: 1.55, marginBottom: 10 }}>
            {t('im.li.h1')}
            <b>Settings &amp; Privacy → Data privacy → Get a copy of your data</b>
            {t('im.li.h2')}
            <b>Posts / Shares</b>
            {t('im.li.h3')}
            <b>Shares.csv</b>
          ...
          <input
            type="file"
            accept=".csv,text/csv"
            onChange={onLinkedInFile}
            style={{ fontSize: 12.5, marginBottom: 6, color: 'var(--text-2)' }}
            aria-label={t('im.li.file.aria')}
          />
      ...
      {mode === 'single' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={{ fontSize: 12.5, color: 'var(--text-3)' }}>{t('im.single.hint')}</div>
          <Input
            label={t('im.single.author')}
            id="clip-author"
            name="clip-author"
            autoComplete="off"
            value={sAuthor}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            <Input
              label={t('im.single.reactions')}
              id="clip-reactions"
              name="clip-reactions"
              type="number"
              min={0}
              value={sReactions}
              onChange={(e) => setSReactions(e.target.value)}
            />
            <Input
              label={t('im.single.comments')}
              id="clip-comments"
              name="clip-comments"
              type="number"
              min={0}
              value={sComments}
              onChange={(e) => setSComments(e.target.value)}
            />
          </div>
          <Input
            label={t('im.single.url')}
            id="clip-url"
            name="clip-url"
            autoComplete="off"
            value={sUrl}
      ...
      {prog && (
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginTop: 10 }}>
          <div style={{ flex: 1, height: 8, background: 'var(--surface-2)', borderRadius: 4, overflow: 'hidden' }}>
            <div
              style={{
                width: (prog.total ? Math.round((prog.processed / prog.total) * 100) : 0) + '%',
                height: '100%',
      ...
      {preview && (
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', color: 'var(--text-2)' }}>
            <span>
              {t('im.preview.total')}
              <span className="num">{preview.total}</span>
            </span>
            <span style={{ color: 'var(--positive)' }}>
              {t('im.preview.added')}
              <span className="num">{preview.added}</span>
            </span>
            <span>
              {t('im.preview.dupes')}
              <span className="num">{preview.dupes}</span>
            </span>
            <span>
              {t('im.preview.near')}
              <span className="num">{preview.nearDupes}</span>
            </span>
            <span style={{ color: 'var(--critical)' }}>
              {t('im.preview.rejected')}
              <span className="num">{preview.rejected}</span>
            </span>
          </div>
          {preview.reasons.length > 0 && (
            <ul
              style={{ margin: '8px 0 0', paddingLeft: 16, color: 'var(--text-3)', maxHeight: 120, overflowY: 'auto' }}
            >
